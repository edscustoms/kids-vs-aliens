...
public static class KidsVsAliensAdaptiveIconSetup
{
    private const string BackgroundPath =
        "Assets/Game/UI/AppIcon/IconBackground.png";

    private const string ForegroundPath =
        "Assets/Game/UI/AppIcon/IconForeground.png";

    private const string GeneratedFolder =
        "Assets/Game/UI/AppIcon/Generated";

    [MenuItem("Tools/Kids VS Aliens/Setup Android Adaptive Icons")]
    public static void Setup()
    {
        ...
        Texture2D foreground =
            AssetDatabase.LoadAssetAtPath<Texture2D>(ForegroundPath);

        if (background == null)
        {
            Debug.LogError(
                $"Missing adaptive icon background:\n{BackgroundPath}"
            );
            return;
        }
        ...
        {
            Debug.LogError(
                $"Missing adaptive icon foreground:\n{ForegroundPath}"
            );
            return;
        }
        ...
        PlatformIcon[] icons =
            PlayerSettings.GetPlatformIcons(platform, kind);
        ...
        for (int i = 0; i < icons.Length; i++)
        {
            ...
            string backgroundOutput =
                $"{GeneratedFolder}/IconBackground_{width}x{height}.png";

            string foregroundOutput =
                $"{GeneratedFolder}/IconForeground_{width}x{height}.png";
        ...
        for (int i = 0; i < icons.Length; i++)
        {
            ...
            Texture2D generatedBackground =
                AssetDatabase.LoadAssetAtPath<Texture2D>(
                    backgroundOutput
                );

            Texture2D generatedForeground =
                AssetDatabase.LoadAssetAtPath<Texture2D>(
                    foregroundOutput
                );

            if (generatedBackground == null ||
                generatedForeground == null)
            {
                Debug.LogError(
                    $"Failed loading generated adaptive icon " +
                    $"{width}x{height}."
                );
                return;
            }
    ...
    private static void ResizeAndWrite(
        Texture2D source,
        int width,
        int height,
        string assetPath
