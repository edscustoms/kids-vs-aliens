...
public static class GrenadeThrowMotionReview
{
    ...
    public static void Capture(string label, bool dense = false, bool side = false)
    {
        if (EditorApplication.isPlayingOrWillChangePlaymode)
            throw new InvalidOperationException("Capture motion outside Play Mode.");
        float[] times = dense ? Enumerable.Range(0, 55).Select(i => i / 30f).ToArray() : Samples;
        string folder = Path.GetFullPath("Logs/GrenadeMotion/" + label);
        ...
        foreach (string character in new[] { "Amy", "SportyGranny" })
        {
            ...
            try
            {
                ...
                if (grenade != null && PreviewVisualSafety.IsVisualPrefab(grenade.heldPrefab))
                {
                    held = Object.Instantiate(grenade.heldPrefab, actor.transform);
                    PreviewVisualSafety.ConfigureInstance(held, 0);
                    foreach (var renderer in held.GetComponentsInChildren<Renderer>()) renderer.renderingLayerMask = 1;
                    held.GetComponent<HeldItemGrip>().AttachTo(actor.WeaponSocket);
                }
                float releaseTime = AssetDatabase.LoadAssetAtPath<AnimationClip>(CharacterAnimationSetup.ThrowClipPath).events
                    .Single(e => e.functionName == nameof(CharacterAnimationEventRelay.OnCharacterAnimationEvent)
                ...
                for (int row = 0; row < Moves.Length; row++)
                {
                    ...
                    animator.SetTrigger("ThrowGrenade"); animator.Update(0);
                    float time = 0;
                    for (int column = 0; column < times.Length; column++)
                    {
                        ...
                        if (held != null) held.SetActive(time <= releaseTime);
        ...
        Debug.Log("Grenade motion review captured to " + folder);
    }
}
